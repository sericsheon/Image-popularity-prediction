...
visual_features_earth = np.load("visual_features_b0_e.npy")
visual_features_pics = np.load("visual_features_b0_p.npy")
...
range_pics = max(score_pics) - min(score_pics)
range_earth = max(score_earth) - min(score_earth)
...
gpt2_captions_pics = np.load('gpt2_embeddings_pics.npy')
bert_captions_pics = np.load('bert_embeddings_pics.npy')
xlnet_captions_pics = np.load('xlnet_embeddings_pics.npy')

gpt2_captions_earth = np.load('gpt2_embeddings_earth.npy')
bert_captions_earth = np.load('bert_embeddings_earth.npy')
xlnet_captions_earth = np.load('xlnet_embeddings_earth.npy')

captions_pics = xlnet_captions_pics
captions_earth = bert_captions_earth
...
visual_features_pca = pca_visual.fit_transform(visual_features_pics)
captions =  pca_cap.fit_transform(captions_pics)

images_array = visual_features_pca
additional_data_array= df_pics[['caption_length', 'author_frequency', 'flair', 'is_original_content','is_over_18','is_locked','cos_posted_at','sin_posted_at']]
additional_data_array['sentiment_vader'] = sentiments_vader_pics
additional_data_array['sentiment_flair'] = sentiments_flair_pics
social_features = additional_data_array
captions_array = captions
scores_array = score_pics
...
show_images_for_predictions(least_accurate, title_pics)
